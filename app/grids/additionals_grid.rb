class AdditionalsGrid
  ...
  filter(:_cpf, :string, if: :check_user, header: User.human_attribute_name(:cpf_registration)) do |value, relation, grid|
    relation.by_cpf_cnpj(value)
  end

  filter(:department, :string, if: :check_user, header: User.human_attribute_name(:department)) do |value, relation, grid|
    relation.by_department(value)
  end

  filter(:client_id, :enum, if: :check_admin, select: proc { User.client.order(:fantasy_name).map {|c| [c.fantasy_name, c.id] }}, header: User.human_attribute_name(:client_id), include_blank: I18n.t('model.select_option') ) do |value, relation, grid|
  ...
  column(:client_id, order: :client_id, if: :check_admin, header: User.human_attribute_name(:client_id) ) do |record, grid|
    if record.client
  ...
  column(:_cpf, order: :cpf, if: :check_user, header: User.human_attribute_name(:cpf_registration) ) do |record, grid|
    record.get_document
  end
