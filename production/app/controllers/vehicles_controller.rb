class VehiclesController < ApplicationController
  ...
  def vehicles_by_cost_center_id
    ...
    result_payload = vehicles.map { |v| v.as_json.merge(display_name: v.label_for_os_select) }
    render json: { result: result_payload }, status: :ok
  rescue => e
    Rails.logger.error("[Vehicles#vehicles_by_cost_center_id] ERROR user_id=#{@current_user&.id} cost_center_id=#{params[:cost_center_id]} #{e.class}: #{e.message}")
  ...
  def vehicles_by_client_id
    ...
    Rails.logger.error("[Vehicles#vehicles_by_client_id] ERROR user_id=#{@current_user&.id} client_id=#{params[:client_id]} #{e.class}: #{e.message}")
    render json: { result: [] }, status: :ok
  end
