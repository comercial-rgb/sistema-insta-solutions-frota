class MaintenanceAlertService
  ...
  def self.notify_managers(alert)
    vehicle = alert.vehicle
    plan_item = alert.maintenance_plan_item
    client_id = vehicle.client_id

    managers = User.where(profile_id: [Profile::MANAGER_ID, Profile::ADDITIONAL_ID])
                   .where(client_id: client_id)

    return if managers.empty?

    notification = Notification.new(
      title: "⚠️ Alerta de Manutenção — #{vehicle.board}",
      message: "#{alert.message}\n\nAcesse o sistema para confirmar a abertura da OS.",
      send_all: false,
      display_type: Notification::DISPLAY_BOTH,
      is_important: true,
      requires_acknowledgment: true
    )
    notification.users = managers.to_a
    notification.save!
  rescue => e
    Rails.logger.error("[MaintenanceAlertService] notify_managers error: #{e.message}")
  end
